...
import { doc, getDoc, updateDoc, serverTimestamp, arrayUnion, collection, getDocs } from "firebase/firestore";
...
function RequestLetterForm({ client, onClose }: { client: Client, onClose: () => void }) {
  ...
  const [selectedClientBanks, setSelectedClientBanks] = useState<Bank[]>(client.banks || []);
  ...
  const handleBankSelect = (value: string) => {
    ...
    if (value.includes('|')) {
      ...
      if (bankDetails) {
        ...
        if (accountNumber && selectedClientBanks.length > 0) {
          const clientBank = selectedClientBanks.find(bank => 
            bank.bankName === bankName && bank.accountNumber === accountNumber
          );
          if (clientBank && clientBank.loanType) {
            accountType = clientBank.loanType === "Credit Card" ? "Credit Card Number" : "Loan Account";
          }
        }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Name field - auto-filled and editable */}
        <div>
        ...
        {/* Email - auto-filled and editable */}
        <div>
        ...
        <div>
          ...
          {selectedBank && selectedBank.includes('|') && (
            <p className="text-xs text-green-500 mt-0.5">
              ✅ Auto-detected from selected client account (editable)
            </p>
          )}
        </div>
