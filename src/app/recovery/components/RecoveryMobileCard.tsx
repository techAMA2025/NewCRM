...
export default function RecoveryMobileCard(props: any) {
  const { 
    record, 
    isDarkMode, 
    onUpdateField, 
  ...
  return (
    <Card className={`overflow-hidden border shadow-sm ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
      ...
      <CardContent className="p-4 space-y-4">
        ...
        {/* Actions */}
        <div className={`pt-3 border-t flex justify-end gap-3 ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
          <button
            onClick={() => onEdit(record)}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-[10px] font-bold uppercase rounded-lg hover:bg-blue-700 transition-all shadow-sm"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 text-white text-[10px] font-bold uppercase rounded-lg hover:bg-red-700 transition-all shadow-sm"
          >
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
