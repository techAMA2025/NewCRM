...
// ─── Indian Trademark Class Descriptions (as per Trade Marks Registry of India) ─
const TM_CLASS_DESCRIPTIONS: Record<number, string> = {
  1:  "Chemicals used in industry, science and photography; unprocessed artificial resins, plastics; fire extinguishing compositions; fertilisers; biological preparations for use in industry and science.",
  2:  "Paints, varnishes, lacquers; preservatives against rust and deterioration of wood; colourants, dyes; inks; raw natural resins; metals in foil and powder form for use in painting and printing.",
  3:  "Non-medicated cosmetics and toiletry preparations; non-medicated dentifrices; perfumery, essential oils; bleaching preparations for laundry use; cleaning, polishing, scouring preparations.",
  4:  "Industrial oils and greases; lubricants; dust-absorbing, wetting and binding compositions; fuels and illuminants; candles and wicks for lighting.",
  5:  "Pharmaceuticals, medical and veterinary preparations; sanitary preparations; dietetic food and substances adapted for medical use; disinfectants; pesticides; herbicides.",
  6:  "Common metals and their alloys; metal building and construction materials; transportable buildings of metal; non-electric cables and wires of common metal; ironmongery, small items of metal hardware; metal containers; safes.",
  7:  "Machines; machine tools; power-operated tools; motors and engines (except for land vehicles); machine coupling and transmission components; agricultural implements (other than hand-operated); incubators for eggs; automatic vending machines.",
  8:  "Hand tools and implements (hand-operated); cutlery; side arms (except firearms); razors.",
  9:  "Scientific, research, navigation, surveying, photographic, cinematographic, audiovisual, optical, weighing, measuring, signalling, detecting, testing, inspecting, life-saving and teaching apparatus and instruments; apparatus and instruments for scientific research; computers; software; electronic apparatus and instruments.",
  ...
  18: "Leather and imitations of leather; animal skins and hides; luggage and carrying bags; umbrellas and parasols; walking sticks; whips, harness and saddlery; collars, leads and clothing for animals.",
  ...
  35: "Advertising; business management and organisation; office functions; retail and wholesale services for pharmaceutical, veterinary and sanitary preparations and related goods.",
  ...
  38: "Telecommunications services; broadcasting services; internet and online services.",
  39: "Transport and travel services; packaging and storage of goods; courier services.",
  ...
  42: "Scientific and technological services; industrial analysis, industrial research and industrial design services; quality control and authentication services; design and development of computer hardware and software.",
  43: "Services for providing food and drink; temporary accommodation; catering services.",
...
const formatInterest = (interest: string): { label: string; colorClass: string; classNum?: number } => {
  ...
  if (lower.includes("trademark")) {
    ...
    const classNum = classMatch ? parseInt(classMatch[1], 10) : undefined
    return {
  ...
  if (lower.includes("patent")) return { label: "Patent", colorClass: "bg-blue-900 text-blue-100 border-blue-700" }
  if (lower.includes("copyright")) return { label: "Copyright", colorClass: "bg-emerald-900 text-emerald-100 border-emerald-700" }
...
const IprKaroLeadsPage = () => {
  ...
  const renderCell = (colId: string, lead: IprKaroLead, w: number) => {
    ...
    switch (colId) {
      ...
      case "interest": {
        ...
        const hasTooltip = !!int.classNum && !!TM_CLASS_DESCRIPTIONS[int.classNum]
        return (
          <td key={colId} className={cls} style={cellStyle}>
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold border uppercase tracking-wider ${hasTooltip ? "cursor-help" : "cursor-default"} ${int.colorClass}`}
              onMouseEnter={hasTooltip ? (e) => {
                const rect = (e.currentTarget as HTMLElement).getBoundingClientRect()
                setTmTooltip({ classNum: int.classNum!, x: rect.left + rect.width / 2, y: rect.bottom + 8 })
              } : undefined}
              onMouseLeave={hasTooltip ? () => setTmTooltip(null) : undefined}
            >
              {int.label}
            </span>
          </td>
  ...
  return (
    <div className="flex h-screen bg-[#F8F5EC] overflow-hidden font-sans">
      ...
      {/* TM Class Tooltip — portal-rendered at fixed viewport position to escape overflow/stacking clipping */}
      {tmTooltip && isMounted && typeof document !== "undefined" && createPortal(
        <div
          className="pointer-events-none fixed z-[9999] w-64 bg-[#1E1A12] text-white rounded-xl shadow-2xl border border-[#D2A02A]/30 p-3 animate-in fade-in duration-150"
          style={{ left: tmTooltip.x, top: tmTooltip.y, transform: "translateX(-50%)" }}
        >
          {/* arrow pointing up */}
          <span className="absolute bottom-full left-1/2 -translate-x-1/2 border-4 border-transparent border-b-[#D2A02A]/40" />
          <p className="text-[8.5px] font-black text-[#D2A02A] uppercase tracking-[0.18em] mb-1.5">
            Trademark Class {tmTooltip.classNum} — India
          </p>
          <p className="text-[10.5px] text-white/80 leading-relaxed font-medium">
            {TM_CLASS_DESCRIPTIONS[tmTooltip.classNum]}
          </p>
        </div>,
        document.body
      )}
