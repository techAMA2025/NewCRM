...
const AssistantDashboard = () => {
  ...
  useEffect(() => {
    const fetchAssistantData = async () => {
      try {
        // Get the current assistant name from localStorage
        const currentAssistant = localStorage.getItem('userName') || "Assistant";
        
        // Fetch ALL clients for assistant dashboard to show overview of problem clients
        const clientsRef = collection(db, 'clients');
        const allClientsQuery = query(clientsRef);
        
        const clientsSnapshot = await getDocs(allClientsQuery);
        
        // Count clients by status and collect only problem clients for recent display
        let activeCount = 0;
        let droppedCount = 0;
        ...
        clientsSnapshot.forEach((doc) => {
          ...
          // Add to recent clients ONLY if they are problem clients (not responding, dropped, or on hold)
          if (recentClientsList.length < 8 && (status === "Not Responding" || status === "Dropped" || status === "On Hold")) {
          ...
          // Check for pending letters - all clients where request_letter is false or not present
        ...
        // Set recent clients (problem clients only)
        ...
        const tasksQuery = query(
          tasksRef,
          where("assigneeName", "==", currentAssistant)
        );
    ...
    const checkPendingTasks = () => {
      ...
      if (pendingTasks.length > 0) {
        ...
        const cleanupNotification = () => {
          ...
          document.title = "Assistant Dashboard - CRM";
        };
  ...
  const navigateToClients = (statusFilter: string) => {
    router.push(`/clients?status=${encodeURIComponent(statusFilter)}`)
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-8 text-gray-200">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">Assistant Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-xl font-semibold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">Today's Reminders</h2>
          ...
          <div className="mt-4 text-right">
            <a href="/reminders" className="text-sm text-cyan-400 hover:text-cyan-300 transition-colors">
              View all reminders →
            </a>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-gray-700">
                {pendingLetters.map((letter) => (
                  <tr key={letter.id} className="hover:bg-gray-700 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button 
                        onClick={() => router.push(`/clients?search=${encodeURIComponent(letter.clientName)}`)}
                        className="text-cyan-400 hover:text-cyan-300 transition-colors"
                      >
                        View Client
                      </button>
